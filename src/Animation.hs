...
data Animated a = Animated
  { elapsedTime :: Float
  , animatedValue :: Animation a
  , idleValue :: a
  }
  deriving Functor

animatable :: a -> Animated a
animatable = Animated 0 mempty

animating :: Animation a -> Animated a
animating anim = Animated 0 anim lastFrame
  where
    lastFrame = snapshot anim (duration anim)


isAnimating :: Animated a -> Bool
isAnimating (Animated {..}) = elapsedTime < duration animatedValue

runAnimated :: Animated a -> a
runAnimated animated | isAnimating animated = snapshot anim t
                     | otherwise            = idleValue animated
  where
    anim = animatedValue animated
    t = elapsedTime animated


animate :: Animation a -> Animated a -> Animated a
animate anim (Animated {..}) = Animated 0 anim idleValue


class Update a where
    update :: Float -> a -> a

instance Update (Animated a) where
    update dt (Animated {..}) = Animated (elapsedTime + dt)
                                         animatedValue
                                         idleValue
